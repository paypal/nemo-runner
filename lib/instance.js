  ...
  var defineSuite = function (config) {
    ...
    runner.on('suite', function (Suite) {
      ...
      Suite.afterEach(function capture() {
        console.log(`afterEach ${!!this.nemo}`);
        return this.nemo.driver.takeScreenshot()
          .then(function () {
            var imageName = `${config.get('profiles:base:reports')}/poop.jpg`;
            console.log(`got the screenshot, ${imageName}`);
            debugger;
            return true;
          }.bind(this))
          .catch(function (err) {
            console.log(`triggered error block: ${err}`);
          });
      });
